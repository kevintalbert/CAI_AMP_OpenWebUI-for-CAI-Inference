...
The Open WebUI for AI Inference Service provides a ready-to-use implementation of an open-source web interface for deploying AI models using Cloudera Machine Learning (CAI). This project leverages Cloudera's AI Inference Service to connect Open WebUI to your models. You can deploy it either as an **AMP** (Applied Machine Learning Prototype) in a CAI project or as a **Cloudera AI (CAI) Inference Application** that uses `entry.sh` as its launch script. The in-browser experience is the same in both cases.
...
2. **Seamless Integration with CAI:** Leverages CAI's AI Inference Service to streamline the deployment and management of machine learning models.

3. **Scalable and Customizable:** Easily scalable within the CAI environment and customizable to fit specific use cases.
...
Deploy from the AMP catalog as shown below. If you do not see it in the community catalog, you can add `https://raw.githubusercontent.com/kevintalbert/CAI_AMP_OpenWebUI-for-CAI-Inference/refs/heads/main/catalog-entry.yaml` to your AMP custom catalog entries.

![](/assets/amp-deployment-1.png)


Enter the required base URL for your inference service instance. You can locate this by navigating to the Model Endpoints in CAI as shown below and copy/paste this into the variable slot in the AMP deployment window.
...
In Cloudera AI, open **Applications** → **Create Application**. Pick your **environment and inference service**, set **Name** and **Subdomain** (for example `open-web-ui` / `chat`), choose **Git** as the source, and point the clone at **this** repository with entry point **`entry.sh`**. A typical Git URL is `https://github.com/kevintalbert/CAI_AMP_OpenWebUI-for-CAI-Inference.git` (use your fork if you maintain one).
